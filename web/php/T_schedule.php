...
if(mysqli_fetch_array(mysqli_query($connect,'select * from t_time where ID = "'.$uid.'"'))){
    $sql='update t_time set MonM="'.$schedule[0].'",MonA="'.$schedule[1].'",TueM="'.$schedule[2].'",TueA="'.$schedule[3].'",WedM="'.$schedule[4].'"
   ...
   where ID="'.$uid.'"';
    ...
    $sql= 'insert into t_time(ID, MonM,MonA,TueM,TueA,WedM,WedA,ThuM,ThuA,FriM,FriA)values("'.$uid.'","'.$schedule[0].'","'.$schedule[1].'","'.$schedule[2].'","'.$schedule[3].'",
